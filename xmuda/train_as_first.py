...
from xmuda.models.build import build_model_2d, build_model_3d, build_model_fusion
...
def train(cfg, output_dir='', run_name=''):
    ...
    print('#Parameters Fusion: {:.2e}'.format(num_params))
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        if 'SCN' in cfg.DATASET_SOURCE.TYPE and 'SCN' in cfg.DATASET_TARGET.TYPE:
            ...
            if cfg.TRAIN.XMUDA.lambda_pl > 0:
                ...
                data_batch_trg['pseudo_label_fusion'] = data_batch_trg['pseudo_label_fusion'].cuda()
        else:
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_src > 0:
            seg_logit_2d = preds_2d_s['seg_logit2'] if cfg.MODEL_2D.DUAL_HEAD else preds_2d_s['seg_logit']  # P_2D-3D
            seg_logit_3d = preds_3d_s['seg_logit2'] if cfg.MODEL_3D.DUAL_HEAD else preds_3d_s['seg_logit']  # P_3D-2D
            xm_loss_src_2d = F.kl_div(F.log_softmax(seg_logit_2d, dim=1),  # Dkl(P_3D||P_2D-3D)
                                      F.softmax(preds_3d_s['seg_logit'].detach(), dim=1),
                                      reduction='none').sum(1).mean()
            xm_loss_src_3d = F.kl_div(F.log_softmax(seg_logit_3d, dim=1),  # Dkl(P_2D||P_3D-2D)
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_trg > 0:
            ...
            loss_fusion_trg = F.cross_entropy(seg_logit_fusion_trg, data_batch_trg['pseudo_label_fusion'],
                                              weight=class_weights)
            train_metric_logger.update(loss_fusion_trg=loss_fusion_trg)
            loss_fusion_trg.backward()

            # fusion representation only used for inference stage
            ...
            pred_fusion_t = {'seg_logit': seg_logit_fusion_trg}
            with torch.no_grad():
                train_metric_fusion.update_dict(pred_fusion_t, data_batch_trg)
