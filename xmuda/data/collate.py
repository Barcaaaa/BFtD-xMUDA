...
def collate_scn_base(input_dict_list, output_orig, output_image=True):
    ...
    for idx, input_dict in enumerate(input_dict_list):
        ...
        if output_pselab:
            ...
            if input_dict['pseudo_label_fusion'] is not None:
                pseudo_label_fusion.append(torch.from_numpy(input_dict['pseudo_label_fusion']))
    ...
    if output_pselab:
        ...
        out_dict['pseudo_label_fusion'] = torch.cat(pseudo_label_fusion, 0) if pseudo_label_fusion else pseudo_label_fusion
    return out_dict
