...
def augment_and_scale_3d(points, scale, full_scale,
    ...
    use_cylinder = False
    if use_cylinder:  # cylinder coordinate
        x, y, z = points.T
        rho = np.sqrt(x ** 2 + y ** 2) * scale
        phi = np.arctan2(y, x) * 180 / np.pi  # corresponds to a split each 1°
        z = z * scale
        coords = np.concatenate((rho[:, None], phi[:, None], z[:, None]), axis=1)
