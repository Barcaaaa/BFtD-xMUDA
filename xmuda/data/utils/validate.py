...
def validate_fusion(cfg,
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            pred_label_voxel_2d = preds_2d['seg_logit'].argmax(1).cpu().numpy()
            pred_label_voxel_3d = preds_3d['seg_logit'].argmax(1).cpu().numpy() if model_3d else None
            pred_label_voxel_fusion = preds_fusion.argmax(1).cpu().numpy()
...
def validate_fusion_gsf(cfg,
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            preds_2d = model_2d(data_batch)
            preds_3d = model_3d(data_batch) if model_3d else None
            preds_fusion = model_fusion(preds_2d['feats_full'],
                                        preds_3d['feats'],
                                        preds_2d['seg_logit_full'].detach(),
                                        preds_3d['seg_logit'].detach(),
                                        data_batch['img_indices'])
...
def validate_ensemble(cfg,
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            probs_ensemble = F.softmax(probs_2d + probs_3d, dim=1) + probs_fusion
            pred_label_voxel_ensemble = probs_ensemble.argmax(1).cpu().numpy() if model_3d else None
