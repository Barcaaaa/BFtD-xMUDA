...
def preprocess(nusc, split_names, root_dir, out_dir,
    ...
    for i, sample in enumerate(nusc.sample):
        ...
        # load lidar points
        # pts = np.fromfile(lidar_path, dtype=np.float32, count=-1).reshape([-1, 5])[:, :3].T  # old_version
        pc_original = LidarPointCloud.from_file(lidar_path)
        pc_ref = pc_original.points  # (4,N)
        pts = pc_ref[0:3, :]  # coord
        intensity = pc_ref[3:, :]  # value
        ...
        pts_img = np.ascontiguousarray(np.fliplr(pts_img))  # pos_index of points lie on the image

        # only use lidar points in the front camera image
        pts = pts[:, pts_valid_flag]  # cartesian space
        pts_cam_coord = pts_cam_coord[:, pts_valid_flag]  # camera space
        intensity = intensity[:, pts_valid_flag]

        # ============================== seg labels ==============================
        seg_label_path = '/data/xmw/nuscenes/lidarseg/v1.0-trainval/'
        lidarseg_labels_filename = seg_label_path + lidar_token + "_lidarseg.bin"
...
if __name__ == '__main__':
    ...
    out_dir = '/data/wuyao/dataset/nuscenes/preprocess_seg_full'  # with depth and intensity info
