...
class NuScenesBase(Dataset):
    ...
    def __init__(self,
        ...
        if pselab_paths:
            ...
            if 'probs_fusion' in self.pselab_data[0].keys():
                probs_fusion = np.concatenate([data['probs_fusion'] for data in self.pselab_data])
                pseudo_label_fusion = np.concatenate([data['pseudo_label_fusion'] for data in self.pselab_data]).astype(
                    np.int)
                pseudo_label_fusion = refine_pseudo_labels(probs_fusion, pseudo_label_fusion)
            else:
                pseudo_label_fusion = None
            ...
            for data_idx in range(len(self.pselab_data)):
                ...
                if pseudo_label_fusion is not None:
                    self.pselab_data[data_idx]['pseudo_label_fusion'] = pseudo_label_fusion[left_idx:right_idx]
                else:
                    self.pselab_data[data_idx]['pseudo_label_fusion'] = None
                left_idx = right_idx
...
class NuScenesSCN(NuScenesBase):
    ...
    def __getitem__(self, index):
        ...
        if self.use_image:
            ...
            depth[img_indices[:, 0], img_indices[:, 1]] = pts_cam_coord[:, 2]  # z value in camera coordinate
        ...
        if self.pselab_data is not None:
            out_dict.update({
                ...
                'pseudo_label_fusion': self.pselab_data[index]['pseudo_label_fusion'][keep_idx][idxs]
            })
...
def test_NuScenesSCN():
    ...
    preprocess_dir = "/data/xmw/xmuda/xmuda/data/nuscenes/preprocess"
    nuscenes_dir = "/data/xmw/xmuda/xmuda/data/nuscenes"  # only front cam images are needed
    ...
    for i in [407]:
        data = dataset[i]
        # print(data.keys())dict_keys(['img', 'img_indices', 'coords', 'feats', 'seg_label'])
        coords = data['coords']
        ...
        print(type(img_indices))  # 二维列表
        # print(seg_label) # 一维列表
...
def compute_class_weights():
    # preprocess_dir = '/data/wuyao/dataset/nuscenes/preprocess_bbox_full'
    preprocess_dir = '/data/wuyao/dataset/nuscenes/preprocess_seg_full'
