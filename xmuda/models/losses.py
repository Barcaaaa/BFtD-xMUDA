...
def SoftCrossEntropy(inputs, target, reduction='average', ignore_index=-100):
    log_likelihood = -F.log_softmax(inputs, dim=1)
    ignore_idx = target != ignore_index
    batch = log_likelihood[ignore_idx].shape[0]
    if reduction == 'average':
        loss = torch.sum(torch.mul(log_likelihood[ignore_idx], target[ignore_idx])) / batch
    else:
        loss = torch.sum(torch.mul(log_likelihood[ignore_idx], target[ignore_idx]))
    return loss