#!/usr/bin/env python
import sys
sys.path.append("/data/wuyao/code/BFtD-xMUDA/")
...
from xmuda.common.solver.build import build_optimizer, build_scheduler, build_scheduler_fusion
...
from xmuda.models.build_us import build_model_2d, build_model_3d, build_model_fusion
...
def train(cfg, output_dir='', run_name=''):
    ...
    # build fusion model
    model_fusion, train_metric_fusion = build_model_fusion(cfg)
    ...
    # add metrics
    train_metric_logger = init_metric_logger([train_metric_2d, train_metric_3d, train_metric_fusion])
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        # ---------------------------------------------------------------------------- #
        # Inference teacher with data_batch_st(src)
        # ---------------------------------------------------------------------------- #
        ...
        seg_logit_fusion_st = model_fusion_ema(preds_2d_st['feats_full'].detach(), preds_3d_st['feats'].detach(),
                                               data_batch_st['img_indices'])
        avg_probs_st = F.softmax(seg_logit_fusion_st, dim=1)

        preds_2d_emat = model_2d_ema(data_batch_trg)
        preds_3d_emat = model_3d_ema(data_batch_trg)
        seg_logit_fusion_emat = model_fusion_ema(preds_2d_emat['feats_full'].detach(), preds_3d_emat['feats'].detach(),
                                                 data_batch_trg['img_indices'])
        avg_probs_emat = F.softmax(seg_logit_fusion_emat, dim=1)
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_src > 0:
            ...
            seg_logit_fusion_src = model_fusion(preds_2d_s['feats_full'].detach(), preds_3d_s['feats'].detach(),
                                                data_batch_src['img_indices'])
            loss_fusion_src = F.cross_entropy(seg_logit_fusion_src, data_batch_src['seg_label'], weight=class_weights)
            train_metric_logger.update(loss_fusion_src=loss_fusion_src)
            ...
            xm_loss_src_2d = F.kl_div(F.log_softmax(seg_logit_2d, dim=1),
                                      F.softmax(seg_logit_fusion_src.detach(), dim=1),
        ...
        pred_fusion_s = {'seg_logit': seg_logit_fusion_src}
        with torch.no_grad():
            train_metric_2d.update_dict(preds_2d_s, data_batch_src)
            train_metric_3d.update_dict(preds_3d_s, data_batch_src)
            train_metric_fusion.update_dict(pred_fusion_s, data_batch_src)

        # backward
        loss_fusion_src.backward()
        ...
        pt_2d = F.log_softmax(preds_2d_t['seg_logit'], dim=1)
        pt_3d = F.log_softmax(preds_3d_t['seg_logit'], dim=1)

        loss_2d_t = []
        loss_3d_t = []
        if cfg.TRAIN.XMUDA.lambda_xm_trg > 0:
            # cross-modal fusion representation
            seg_logit_fusion_trg = model_fusion(preds_2d_t['feats_full'].detach(), preds_3d_t['feats'].detach(),
                                                data_batch_trg['img_indices'])
            ...
            xm_loss_trg_2d = F.kl_div(F.log_softmax(seg_logit_2d, dim=1),
                                      F.softmax(seg_logit_fusion_trg.detach(), dim=1),
            ...
            loss_2d_t.append(cfg.TRAIN.XMUDA.lambda_xm_trg * xm_loss_trg_2d) # add new item into the list
        ...
        loss_cdkd = st_loss_2d + st_loss_3d
        ...
        plt_loss_2d = F.kl_div(pt_2d, avg_probs_emat.detach(), reduction='none').sum(1).mean()
        plt_loss_3d = F.kl_div(pt_3d, avg_probs_emat.detach(), reduction='none').sum(1).mean()
        plt_loss_trg = plt_loss_2d + plt_loss_3d

        lambda_plt = 0.0
        if iteration > 80000:
            lambda_plt = 0.001
            train_metric_logger.update(plt_loss_trg=lambda_plt * plt_loss_trg)

        loss = loss_stu + loss_cons + lambda_plt * plt_loss_trg
        ...
        if cur_iter == 1 or (cfg.TRAIN.LOG_PERIOD > 0 and cur_iter % cfg.TRAIN.LOG_PERIOD == 0):
            logger.info(
                train_metric_logger.delimiter.join(
                    ...
                    lr_f=optimizer_fusion.param_groups[0]['lr'],
