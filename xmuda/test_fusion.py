...
from xmuda.models.build_dn import build_model_2d, build_model_3d, build_model_fusion
...
from xmuda.data.utils.validate import validate_fusion, validate_fusion_gsf
from xmuda.data.utils.validate_vis import viz_fusion
...
def parse_args():
    ...
    parser.add_argument('--fusion', type=str, help='path to checkpoint file of the fusion model', default='')

    parser.add_argument('--pselab', action='store_true', help='generate pseudo-labels')
    parser.add_argument('--viz', action='store_true', help='visualize pseudo-labels')
    parser.add_argument(
        'opts',
...
def test(cfg, args, output_dir=''):
    ...
    # build fusion model
    model_fusion = build_model_fusion(cfg)[0]
    ...
    checkpointer_fusion = CheckpointerV2(model_fusion, save_dir=output_dir, logger=logger)
    if args.fusion:
        # load weight if specified
        weight_path = args.fusion.replace('@', output_dir)
        checkpointer_fusion.load(weight_path, resume=False)
    else:
        # load last checkpoint
        checkpointer_fusion.load(None, resume=True)
    ...
    model_3d.eval()
    model_fusion.eval()
    if args.viz:  # generate pseudo labels and visualize results
        viz_fusion(cfg, model_2d, model_3d, model_fusion, test_dataloader, test_metric_logger,
                   pselab_path=pselab_path, output_dir=output_dir)
    else:
        # validate_fusion(cfg, model_2d, model_3d, model_fusion, test_dataloader, test_metric_logger,
        #                 pselab_path=pselab_path)
        validate_fusion_gsf(cfg, model_2d, model_3d, model_fusion, test_dataloader, test_metric_logger,
                        pselab_path=pselab_path)
        # validate_ensemble(cfg, model_2d, model_3d, model_fusion, test_dataloader, test_metric_logger,
        #                 pselab_path=pselab_path)
